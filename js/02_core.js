 ...
 * Version: 1.9 (Fixed Syntax + Robustness - 2026-07-14)
 */
'use strict';
...
window.ARK = {
    ...
    get: function(key) {
        if (!window.BLACKBOARD) {
            console.warn('[ARK] BLACKBOARD not initialized');
    ...
    _loadRiskCurveAsync: async function(force = false) {
        console.log("[ARK] Starting risk curve hydration...");
        const updateTs = new Date().toISOString();

        try {
            const nodes = {
                "Liquidity": this._mapToLiquidityNode({}),
                "Credit": this._mapToCreditNode({}),
                "EquityBreadth": this._mapToEquityBreadthNode({}),
                "HighBeta": this._mapToHighBetaNode({}),
                "BTCBasis": this._mapToBTCBasisNode({})
            };
            ...
            console.log("[ARK] Hydration completed successfully");
            return data;
        } catch (err) {
    ...
    refresh: async function() {
        console.log("[ARK] Full refresh triggered");
...
console.log("✅ ARK + BLACKBOARD Core v1.9 Loaded Successfully.");