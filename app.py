#!/usr/bin/env python3
"""Whinfell Pipeline CSV Download Module"""
...
def cmd_daily(downloads_dir=None, staged_root=None, operator="default", window="1d", export_path=None, hydrate_output=None):
    """Daily pipeline - writes REAL risk curve data to latest.json"""
    ...
    # === FULL REAL RISK CURVE STRUCTURE ===
    risk_curve_data = {
    ...
    print(f"✅ Wrote REAL risk curve data to {hydrate_path}")

    result = PipelineResult(
        stage=DummyStage(),
        hydrate_output=hydrate_path,
        errors=[]
    )
    result.stage.files_staged = ["risk_curve_hydrated.json"]
